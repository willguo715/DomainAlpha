...
# 4. 读取腾讯云CSV文件并分批处理
# ==========================================
def load_and_batch_tencent_domains(csv_file_path: str, batch_size: int = 200, shuffle: bool = True):
    """
    读取腾讯云CSV文件，提取域名和价格，打乱后分批返回
    
    Args:
        csv_file_path: CSV文件路径（如 "2025-12-26_tencent.csv"）
        batch_size: 每批返回的数量（默认200）
    ...
    try:
        with open(csv_file_path, 'r', encoding='utf-8-sig') as f:
            ...
            # 验证必需的列是否存在
            if '域名' not in reader.fieldnames or '当前价格' not in reader.fieldnames:
                raise ValueError(f"CSV文件缺少必需的列。需要的列: 域名, 当前价格")
            
            # 读取数据
            for row_num, row in enumerate(reader, start=2):
                domain = row.get('域名', '').strip()
                price = row.get('当前价格', '').strip()
...
# 5. 格式化批次数据
# ==========================================
def format_batch_for_model(batch: list, batch_num: int) -> str:
    """
    将一批域名和价格数据格式化为模型输入文本
    ...
    Returns:
        str: 格式化后的文本
    """
    lines = [f"第 {batch_num} 批域名列表（共 {len(batch)} 个）：\n"]
    lines.append("| 域名 | 当前价格 |")
    lines.append("| :--- | :--- |")
    
    for domain, price in batch:
        lines.append(f"| {domain} | {price} |")
...
# 6. 解析模型返回结果
# ==========================================
def parse_model_response(response: str) -> list:
    """
    从模型返回的文本中解析出推荐结果
    ...
    # 查找Markdown表格
    # 匹配格式：| 序号 | 域名 | 当前价格 | 预计出手价格 | 推荐理由 | 投资等级 |
    table_pattern = r'\|[\s:---]+\|'
    ...
    for line in lines:
        ...
        # 检测表头
        if '|' in line and ('序号' in line or '域名' in line):
            header_found = True
            in_table = True
            continue
...
def send_result_email(csv_file: str, date_str: str, total_token_stats: dict):
    ...
    try:
        ...
        print(f"[邮件] 准备发送邮件，主题: 腾讯云域名分析结果 - {date_str}")
        print(f"[邮件] 附件文件: {csv_file}")
        ...
        result = send_email_simple(
            subject=f"腾讯云域名分析结果 - {date_str}",
            content=html_content,
            content_type="html",
            attachments=[csv_file],  # 添加CSV文件作为附件
            use_default_recipients=True
        )
        
        if result:
            print(f"[邮件] ✅ 腾讯云域名分析结果邮件发送成功（包含附件: {csv_file}）")
        else:
            raise Exception("邮件发送函数返回False，发送可能失败")
...
# 11. 发送错误邮件
# ==========================================
def send_error_email(error_info: str, date_str: str = None, traceback_info: str = None):
    """
    发送错误报告邮件
    ...
    try:
        ...
        send_email_simple(
            subject=f"腾讯云域名分析系统错误报告 - {date_str}",
            content=html_content,
            content_type="html",
            use_default_recipients=True
        )
...
def process_domains_batch(date_str: str = None):
    """
    完整的域名处理流程：
    1. 读取CSV文件，分批处理（每批200个，返回10个推荐）
    2. 汇总所有批次的推荐
    3. 最终筛选得到10个推荐
    4. 保存到结果CSV文件
    
    Args:
        date_str: 日期字符串，格式为 "YYYY-MM-DD"（如 "2025-12-26"）
                 如果为None，使用当前日期
    """
    # 1. 获取CSV文件名
    if date_str is None:
    ...
    # 2. 统计信息
    all_data = list(load_and_batch_tencent_domains(csv_file, batch_size=200, shuffle=True))
    total_count = sum(len(batch) for batch in all_data)
    total_batches = len(all_data)
    batch_size = 200
    
    print(f"[统计] 总域名数: {total_count}, 每批: {batch_size}, 共 {total_batches} 批\n")
    ...
    for batch_num, batch in enumerate(load_and_batch_tencent_domains(csv_file, batch_size=200, shuffle=True), 1):
        batch_count = batch_num
        print(f"\n[批次 {batch_num}/{total_batches}] 处理中（共 {len(batch)} 个域名）...")
...
{summary_text}"""
    ...
    try:
        ...
        send_error_email(error_info, date_str, traceback.format_exc())
        ...
        try:
            ...
            print(f"[错误] 发送降级方案结果邮件失败: {email_err}")
            import traceback
            print(f"[错误详情] {traceback.format_exc()}")
    ...
    try:
        ...
        # 尝试发送错误邮件通知
        try:
            error_info = f"腾讯云域名分析完成，但结果邮件发送失败。\n\n错误信息: {str(e)}\n\n结果文件已保存到: {output_file}\n\n请手动查看结果文件。"
            send_error_email(error_info, date_str, error_detail)
        except:
            print("[错误] 发送错误通知邮件也失败了")
...
if __name__ == "__main__":
    ...
    try:
        ...
        try:
            if date_str is None:
                date_str = datetime.now().strftime('%Y-%m-%d')
            send_error_email(error_info, date_str, error_traceback)
